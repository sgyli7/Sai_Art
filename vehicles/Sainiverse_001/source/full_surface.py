...
def family(p):
    ...
    if mat=='deck_steel':return 'floor'
...
def uv(mesh,part):
    ...
    for k,ij in enumerate([[1,2],[0,2],[0,1]]):
        ...
        if meta['family']=='floor' and k==2:
            out[active,0]=meta['tile']+.00001+(v[active][:,ij[0]]-lo[ij[0]])/4096.
            out[active,1]=span[ij[1]]-(v[active][:,ij[1]]-lo[ij[1]])
        else:
