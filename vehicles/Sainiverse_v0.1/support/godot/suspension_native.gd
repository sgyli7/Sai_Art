...
func _physics_process(dt:float)->bool:
	...
	if steering!=null:
		...
		var state:Dictionary=steering.update(speed_request,speed,0. if elapsed<10 else float(options.curvature),float(world[0])-float(neutral[0]),float(world[1])-float(neutral[1]),atan2(-leader.global_basis.x.z,leader.global_basis.x.x),dt)
