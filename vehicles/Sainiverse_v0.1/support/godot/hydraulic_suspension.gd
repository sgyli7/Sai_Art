...
func step(q:Array,dq:Array,dt:float)->void:
	var capacity:float=float(c.accumulator_capacity_m3)
	var target:float=float(c.target_wheel_coordinate_m)
	var deadband:float=float(c.level_deadband_m)
	var flow_gain:float=float(c.level_flow_gain_m2_s)
	var flow_limit:float=float(c.level_flow_limit_m3_s)
	var header_pressure:float=float(c.header_pressure_Pa)
	var pump_limit:float=float(c.pump_power_limit_W)
	var reservoir_capacity:float=float(c.reservoir_capacity_m3)
	var pressure_limit:float=float(c.pressure_limit_Pa)
	...
	for i in names.size():
		...
		var relief:float=maxf(float(vmin[i])-raw,0.);var makeup:float=maxf(raw-capacity,0.)
		mechanical_volume.append(clampf(raw,float(vmin[i]),capacity))
		reservoir+=relief-makeup-annulus*delta;heat_J+=relief*pressure_limit;relief_volume_m3+=relief;makeup_volume_m3+=makeup
		limited=limited or relief>1e-12
		var error:float=float(q[i])-target
		error=error-signf(error)*deadband if absf(error)>deadband else 0.
		var flow:float=clampf(flow_gain*error,-flow_limit,flow_limit)
		flow=maxf(flow,-(capacity-float(mechanical_volume[i]))/dt);flows.append(flow);requested+=header_pressure*maxf(flow,0.)
	var scale:float=minf(1.,pump_limit/maxf(requested,1.));var positive:=0.0;var negative:=0.0
	for i in names.size():
		if float(flows[i])>0:flows[i]*=scale
		positive+=maxf(flows[i],0.)*dt;negative+=maxf(-float(flows[i]),0.)*dt
	var add_scale:float=minf(1.,maxf(reservoir,0.)/maxf(positive,1e-30));var bleed_scale:float=minf(1.,maxf(reservoir_capacity-reservoir,0.)/maxf(negative,1e-30))
	pump_power_W=0.
	for i in names.size():
		var flow:float=float(flows[i])*(add_scale if float(flows[i])>0 else bleed_scale)
		var pump_work:float=header_pressure*maxf(flow,0.)*dt;pump_power_W+=pump_work/dt
		var raw:float=float(mechanical_volume[i])-flow*dt;var relief:float=maxf(float(vmin[i])-raw,0.)
		volume[i]=clampf(raw,float(vmin[i]),capacity);reservoir+=relief-flow*dt;relief_volume_m3+=relief
		...
		var gas:float=clampf(float(gas_constant[i])/pow(volume[i],gamma)-atm,0.,pressure_limit)
		if float(volume[i])>=capacity-1e-12:gas=0.
		var spring:float=-area*gas;var wanted:float=spring-float(damping[i])*float(dq[i])
		forces[i]=clampf(wanted,-area*pressure_limit,annulus*pressure_limit)
	...
	for i in names.size():current_inventory+=capacity-float(volume[i])-rod_area*float(q[i])
	...
	valid=reservoir>=0. and reservoir<=reservoir_capacity and max_inventory_error_m3<1e-7
