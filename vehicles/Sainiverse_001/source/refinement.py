...
def refine(a,box,rod,add,role,out):
    ...
    for p in parts:
      ...
      if p['name'].endswith('receiver_seat'):
        ...
        cut=tm.creation.cylinder(radius=.685,height=5,sections=32);cut.apply_transform(tm.geometry.align_vectors([0,0,1],[1,0,0]));cut.apply_translation([-1,-7.5,14.90])
    ...
    for p in parts:
      if p['name'].endswith('rooftop_receiver'):
        ...
        for x,radius in [(-3,0.01),(-2.95,.26),(-2.82,.48),(-2.60,.62),(-2.33,r),(.33,r),(.60,.62),(.82,.48),(.95,.26),(1,.01)]:
          rings.extend([[x,-7.5+radius*math.cos(t),14.90+radius*math.sin(t)] for t in np.linspace(0,2*math.pi,24,endpoint=False)])
        for j in range(9):
          for k in range(24):
            n=(k+1)%24;faces.extend([[j*24+k,(j+1)*24+k,(j+1)*24+n],[j*24+k,(j+1)*24+n,j*24+n]])
        rings.extend([[-3,-7.5,14.90],[1,-7.5,14.90]])
        for k in range(24):
