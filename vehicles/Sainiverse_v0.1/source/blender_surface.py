...
def apply(mat,shader,base_image,normal_image,label_image):
    ...
    iswall=calc('MULTIPLY',calc('GREATER_THAN',index,7.5),calc('LESS_THAN',index,9.5))
    isceiling=calc('GREATER_THAN',index,13.5)
    metric=calc('ADD',isfloor,calc('ADD',iswall,isceiling))
    sx=calc('ADD',8,calc('ADD',calc('MULTIPLY',iswall,-6.8),calc('MULTIPLY',isceiling,-5.92)))
    sy=calc('ADD',4,calc('ADD',calc('MULTIPLY',iswall,-1.6),calc('MULTIPLY',isceiling,-1.05)))
    # Same body-local metric projection as Godot; no packed fractional metres.
    coord=nodes.new('ShaderNodeTexCoord');position=nodes.new('ShaderNodeSeparateXYZ');links.new(coord.outputs['Object'],position.inputs[0])
    geom=nodes.new('ShaderNodeNewGeometry');transform=nodes.new('ShaderNodeVectorTransform');transform.vector_type='NORMAL';transform.convert_from='WORLD';transform.convert_to='OBJECT';links.new(geom.outputs['Normal'],transform.inputs[0])
    normal_axis=nodes.new('ShaderNodeSeparateXYZ');links.new(transform.outputs[0],normal_axis.inputs[0])
    nx,ny,nz=[calc('ABSOLUTE',normal_axis.outputs[i]) for i in range(3)]
    onx=calc('MULTIPLY',calc('GREATER_THAN',nx,ny),calc('GREATER_THAN',nx,nz))
    ony=calc('MULTIPLY',calc('GREATER_THAN',ny,nx),calc('GREATER_THAN',ny,nz))
    mx=calc('ADD',position.outputs['X'],calc('MULTIPLY',onx,calc('SUBTRACT',position.outputs['Y'],position.outputs['X'])))
    orientation=calc('ADD',1,calc('ADD',calc('MULTIPLY',onx,calc('SUBTRACT',calc('SIGN',normal_axis.outputs['X']),1)),calc('MULTIPLY',ony,calc('SUBTRACT',calc('MULTIPLY',-1,calc('SIGN',normal_axis.outputs['Y'])),1))))
    mx=calc('MULTIPLY',mx,orientation)
    my=calc('MULTIPLY',-1,calc('ADD',position.outputs['Y'],calc('MULTIPLY',calc('ADD',onx,ony),calc('SUBTRACT',position.outputs['Z'],position.outputs['Y']))))
    floorx=calc('ADD',.016,calc('MULTIPLY',.968,calc('FRACT',calc('DIVIDE',mx,sx))))
    floory=calc('ADD',.016,calc('MULTIPLY',.968,calc('FRACT',calc('DIVIDE',my,sy))))
    def choose(a,b):return calc('ADD',a,calc('MULTIPLY',calc('SUBTRACT',b,a),metric))
