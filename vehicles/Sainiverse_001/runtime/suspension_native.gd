...
func _build()->void:
	...
	for item in spec.bodies:
		...
		body.gravity_scale=0.;body.linear_damp=0.;body.angular_damp=0.;body.can_sleep=false
...
func _physics_process(dt:float)->bool:
	...
	if steering!=null:
		...
		var state:Dictionary=steering.update(speed_request,speed,0. if elapsed<10 else float(options.curvature),0. if str(options.get("mode",""))=="manual" else float(world[0])-float(neutral[0]),0. if str(options.get("mode",""))=="manual" else float(world[1])-float(neutral[1]),0. if str(options.get("mode",""))=="manual" else atan2(-leader.global_basis.x.z,leader.global_basis.x.x),dt)
	...
	for body:RigidBody3D in bodies.values():body.apply_central_force(Vector3.DOWN*body.mass*9.81)
	...
	if hydraulics!=null:
		...
		for link in hydraulic_links:
			...
			coordinates.append((pb-pa).dot(axis));rates.append((point_velocity(link.body,pb)-point_velocity(link.parent,pa)).dot(axis))
	...
	for link in links:
		...
		var axis:Vector3=parent.global_basis*link.axis
		var pa:Vector3=parent.global_transform*link.a;var pb:Vector3=body.global_transform*link.b
		...
		qvalues[item.name]=q
		if bool(item.get("lift",false)):continue
	...
	var points:Array=[];var total_load:=0.0;var supported:=0
	for item in contacts:
		...
		points.append({"body":body,"point":point,"normal":normal,"forward":forward,"lateral":lateral,"velocity":velocity,"load":load,"speed":longitudinal_speed})
		total_load+=load
	...
	for p in points:
		...
		p.leverage=(p.point-com(leader)).cross(p.forward).y;leverage_mean+=float(p.leverage)*float(p.share)
	...
	if count>=roundi(float(options.seconds)*200) or failed:
		var rms:Array=[]
		...
		var out:Dictionary={"engine":"Godot "+str(Engine.get_version_info().string),"physics_engine":ProjectSettings.get_setting("physics/3d/physics_engine"),
			...
			"peak_acceleration_times_s":peak_times,"max_all_step_lateral_path_error_m":route_max,"maximum_all_step_joint_anchor_residual_m":maximum_anchor_residual,
			...
			"scope":"Native Jolt scalar serial joints with springs and finite-power track forces. Explicit physical gravity and reduced compliant belt contacts. Provisional parameters, no learned policy or visual/FPS claim."}
