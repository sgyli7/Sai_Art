"""Default Sainiverse 001 native driving entry in the existing game runtime."""
from pathlib import Path
import argparse,subprocess,os,re,json,time,fcntl,signal
O=Path(__file__).resolve().parents[1];R=O/'support'
p=argparse.ArgumentParser();p.add_argument('--game',type=Path,default=Path(os.environ.get('SAI_GODOT_PROJECT','.')));p.add_argument('--mode',choices=['manual','parked','straight','turn','lift_cycle','traverse'],default='manual');p.add_argument('--terrain',choices=['flat','rough','ramp','ditch','hills','polar'],default='flat');p.add_argument('--seconds',type=float,default=0);p.add_argument('--view',default='whole');p.add_argument('--theme',choices=['black','desert','white','blue'],default='black');p.add_argument('--headless',action='store_true');p.add_argument('--prepare-only',action='store_true');p.add_argument('--pv',action='store_true');p.add_argument('--capture',action='store_true');p.add_argument('--clean-capture',action='store_true');p.add_argument('--output',type=Path);a=p.parse_args()
runtime=a.game/'results/leviathan003/runtime';project=runtime/'project.godot';assert project.exists()
lock=open(runtime/'black_warrior.lock','w');fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
...
speed=27.777778 if a.mode=='straight' else 7. if a.mode=='turn' else 8. if a.mode=='traverse' else 0.
