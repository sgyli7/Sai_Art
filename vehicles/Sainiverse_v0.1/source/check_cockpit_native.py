...
import json
O=Path(__file__).resolve().parents[1];p=O/'reports/r032_cockpit_control_test_v2'
...
assert {x['control'] for x in events}==expected and all(x['accepted'] for x in events)
assert all(sum(x['control']==name for x in events)==2 for name in expected)
assert all(max(x['values'][k] for x in rows)>.15 for k in rows[0]['values'])
...
assert max(x['slew_rad'] for x in q)>.2 and max(x['luff_rad'] for x in q)>.15 and max(x['extension_m'] for x in q)>.8 and max(x['paid_length_m'] for x in q)>5
...
out=dict(seconds=195,controls=17,physical_button_events=10,selected_crane=3,selected_lift=3,all_six_lifts_deployed_and_retracted=True,report=str(p.relative_to(O)),scope='Native dynamics and command-chain trace; no robot arm policy, structural qualification or subjective artwork approval.')
