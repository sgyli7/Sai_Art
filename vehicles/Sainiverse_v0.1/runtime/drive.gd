...
func _release_quick_robot_camera()->void:
	...
	var robot_target:Vector3=_selected_robot_position()+Vector3.UP*(.2 if active_robot_kind in ["sai001","sai002"] else .1)
...
func _manual_robot_camera(target:Vector3)->void:
	...
	if active_quick_location==1:
		...
		if moved or Input.is_action_pressed("sainiverse_forward") or Input.is_action_pressed("sainiverse_reverse") or Input.is_action_pressed("sainiverse_left") or Input.is_action_pressed("sainiverse_right"):
			...
			camera.global_position=whole+bodies.front.global_basis*panorama_offset
...
func _write_visual_report()->void:
	...
	switches.store_string(JSON.stringify({"history":robot_switch_history,"active_robot":active_robot_kind,"quick_travel":quick_travel_history},"  "));switches.close()
	if patrol!=null:
