...
for step in range(round(args.seconds/.005)):
 for lift in p['boarding_lifts']:
  ...
  for i,n in enumerate(names):
   q,v=c.state(n);gravity=0 if i==0 else -9.81*(sum(lift['masses'][i:])+lift['ramp']['mass_kg'])*vertical
   e.d.ctrl[e.m.actuator(n).id]=np.clip(-80000*q-14000*v+gravity,-60000,60000)
