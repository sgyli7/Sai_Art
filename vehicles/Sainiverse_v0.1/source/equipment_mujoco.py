...
class Equipment:
 ...
 def __call__(self,dt):
  ...
  for c in self.rig['cranes']:
   ...
   self.advance(c['luff'],.30 if working else 0.,.035,dt);q,v=self.state(c['luff'])
   requested=14e6*(self.target[c['luff']]-q)-2.8e6*v-self.gravity(c['luff'],[c['luff'],c['extend'],c['hook']])
   ...
   self.servo(c['extend'],.65 if working else 0.,.2,650e3,320e3,600e3,dt,[c['extend'],c['hook']])
   goal=max(1.3,c['paid_length_m']-1.4) if working else c['paid_length_m'];self.paid[c['name']]+=np.clip(goal-self.paid[c['name']],-.4*dt,.4*dt)
   ...
   if self.step_count%20==0:self.rows.append(dict(time=t,name=c['name'],slew_rad=float(self.state(c['slew'])[0]),luff_rad=float(q),extension_m=float(self.state(c['extend'])[0]),cable_force_N=float(tension),cylinder_force_N=float(f),paid_length_m=float(self.paid[c['name']])))
