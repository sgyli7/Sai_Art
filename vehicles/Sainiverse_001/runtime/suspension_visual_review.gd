...
func _process(_dt:float)->bool:
	...
	last_frame_usec=now
	if str(options.get("pv","false"))=="true" and elapsed>=32.+float(pv_frames)/15. and pv_frames<150 and not capture_running:
		_capture("pv_%04d"%pv_frames);pv_frames+=1
	if str(options.get("capture","false"))=="true":
		for when in ([9.,24.,40.,72.] if str(options.get("mode",""))=="lift_cycle" else [9.,24.]):
