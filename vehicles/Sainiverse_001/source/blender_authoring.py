...
for name,h in a['colors'].items():
    ...
    if name.startswith('art_'):
        ...
        if name=='art_workbay_wall':
            ...
            normal_sep=nodes.new('ShaderNodeSeparateXYZ');links.new(trans.outputs[0],normal_sep.inputs[0]);mask=calc('LESS_THAN',normal_sep.outputs['Y'],-.5)
    ...
    if name=='deck_steel':
        nodes=mat.node_tree.nodes;links=mat.node_tree.links
        geom=nodes.new('ShaderNodeNewGeometry');trans=nodes.new('ShaderNodeVectorTransform');trans.vector_type='NORMAL';trans.convert_from='WORLD';trans.convert_to='OBJECT';links.new(geom.outputs['Normal'],trans.inputs[0])
        sep=nodes.new('ShaderNodeSeparateXYZ');links.new(trans.outputs[0],sep.inputs[0]);ab=nodes.new('ShaderNodeMath');ab.operation='ABSOLUTE';links.new(sep.outputs['Z'],ab.inputs[0]);side=nodes.new('ShaderNodeMath');side.operation='LESS_THAN';side.inputs[1].default_value=.65;links.new(ab.outputs[0],side.inputs[0])
    ...
    for face in mesh.polygons:
        ...
        for li in face.loop_indices:
            if meta:
                ...
                coord=[meta['tile']+.00001+(v[ij[0]]-lo[ij[0]])/4096,span[ij[1]]-(v[ij[1]]-lo[ij[1]])] if meta['family']=='floor' and axis==2 else [(meta['tile']%2 if meta['family']=='floor' else meta['tile'])+.02+q[0]*.96,.02+(1-q[1])*.96]
            else:coord=[-1.,-1.]
...
scene['skin']='黑色';scene['vehicle_name']='Sainiverse 001 / 赛思黑 001';scene['art_revision']='r030';scene['scope']='Editable authored model. Godot enamel/ink shader lives in assets. Physical validation is in reports; no learned robot policy.'
text=bpy.data.texts.new('READ ME — Sainiverse 001');text.write('Source SI +X forward +Y left +Z up. Separate body groups and per-part materials. See README.md for driving and validation. Reference-derived conceptual geometry, not a CAD manufacturing qualification.')
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'source/Sainiverse_001.blend'))
