...
var previous_wheel_targets:Array=[0.0,0.0,0.0,0.0]
...
var rough_residual_enabled:=false
...
func _init(profile_path := "") -> void:
	...
	if not str(profile_path).is_empty():
		...
		if schema==2:
			var training: Dictionary = profile.get("training",{})
			var trained_hz := int(training.get("control_hz",50))
			if trained_hz != roundi(1.0/CONTROL_DT):
				_fail("Sai stair policy training/control rate mismatch: trained %d Hz, runtime %d Hz" % [trained_hz,roundi(1.0/CONTROL_DT)])
				return
			...
			rough_residual_enabled=task_space_skills and bool(profile.get("rough_residual_enabled",false))
...
func _validate_settings() -> void:
	for item in [["speed",0.02,0.35 if task_space_skills else 0.16],["lift_height",0.0,0.1],["leg_scale",0.0,0.6],["min_crouch",0.0,1.0]]:
	...
	var stop_slew:=float(stair_settings.wheel_stop_slew_rad_s2)
	if stop_slew<0.0 or stop_slew>60.0:_fail("Invalid wheel stop slew")
...
func reset() -> void:
	...
	delivered_target.fill(0.0)
	crouch = 0.0
	desired_heading = null
	previous_wheel_targets=[0.0,0.0,0.0,0.0]
	motion_origin = null
...
func command(state: Dictionary) -> Dictionary:
	if state.get("robot_id") not in ["Sai_Agent_001","Sai_Agent_002"] or state.get("physics_owner") != "Godot/Jolt":
	...
	# v6 uses sensed edge geometry. A profile id alone must not turn every
	# millimetre of low-relief terrain into a stair speed cap.
	var relevant := _step_in_wheel_path(state,requested[2]>.5,not task_space_skills)
	var descending: bool = bool(state.get("contact_following_descent",false)) and _descending_in_path(state) and requested[0]>.015
	if descending and not task_space_skills: requested[0]=minf(.16,requested[0])
	...
	if stairs and not task_space_skills: requested[0]=minf(requested[0],float(stair_settings.speed))
	...
	return _limit_delivered_target(stance_impedance.apply(result,state) if stance_impedance!=null else result)
...
func _task_space_skill_command(state:Dictionary,requested:Array,phase_time:float)->Dictionary:
	...
	# Log-only: raw ONNX channel 15 before the relevance gate clears it.
	var raw_skill_intensity:=float(inferred[15])
	if not skill_relevant and not rough_residual_enabled:skill_action[15]=0.0
	...
	var result:={"mode":"transport","stage":stage,"target_leg":target,"wheel_speed":[target[3],target[7],target[11],target[15]],
		...
		"raw_skill_intensity":raw_skill_intensity,"skill_relevant":skill_relevant,
		...
		"rough_residual_enabled":rough_residual_enabled,
		"wheel_stop_slew_rad_s2":float(stair_settings.wheel_stop_slew_rad_s2),
	...
	for wheel_index in range(4):
		var actuator_index:int=wheel_index*4+3
		var wheel_target:float=float(result.target_leg[actuator_index])
		if float(stair_settings.wheel_stop_slew_rad_s2)>0.0 and absf(requested[0])<.015 and absf(requested[1])<1e-5:
			wheel_target=move_toward(float(previous_wheel_targets[wheel_index]),0.0,float(stair_settings.wheel_stop_slew_rad_s2)*CONTROL_DT)
			result.target_leg[actuator_index]=wheel_target
		previous_wheel_targets[wheel_index]=wheel_target
	result["wheel_speed"]=[result.target_leg[3],result.target_leg[7],result.target_leg[11],result.target_leg[15]]
	return _limit_delivered_target(stance_impedance.apply(result,state) if stance_impedance!=null else result)

func _limit_delivered_target(result: Dictionary) -> Dictionary:
	# All upstream branches and suspension/feed-forward contributions share
	# one actuator-facing slew bound, including mode transitions.
	var target: Array = result.target_leg.duplicate()
	for i in range(16):
		if i % 4 == 3: continue
		var previous_value: float = float(delivered_target[i])
		target[i] = previous_value + clampf(float(target[i]) - previous_value,
			-10.0 * CONTROL_DT, 10.0 * CONTROL_DT)
	for i in range(16): delivered_target[i] = float(target[i])
...
static func targets_task_skill(action:PackedFloat32Array,base_target:Array,last_residual:Array)->Array:
	...
	for i in range(16):
		...
		result[i]=float(base_target[i])+float(last_residual[i])+clampf(desired_residual-float(last_residual[i]),-float(slew[i%4])*CONTROL_DT,float(slew[i%4])*CONTROL_DT)
	return result
...
static func targets(action: PackedFloat32Array, input_command: Array, effective_crouch: float) -> Array:
	...
	for leg in range(4):
		...
		result[base+3]=SIDES[leg]*((input_command[0]-input_command[1]*SIDES[leg]*.146)/.048+3.0*filtered[base+3])
	return result
