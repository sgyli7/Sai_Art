...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'candidates/r025_access'
...
scene['scope']='Complete three-module neutral model with hollow bridge and robot-height interior. Six separate door bodies and modeled hinge drives; saved closed neutral pose. Inspection robots are real home-pose visuals only; no boarding, reach or policy claim.'
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'source/Leviathan_003_access.blend'))

view((21.4,-7.6,13.15),(19.15,-3.1,12.45),34)
scene.render.filepath=str(OUT/'reports/door_closed_authoring.png');bpy.ops.render.render(write_still=True)
access=json.loads((OUT/'source/access.json').read_text())
for door in access['doors']:
    parents[door['name']].rotation_euler.z=door['side']*door['limits_rad'][1]
scene.render.filepath=str(OUT/'reports/door_open_inspection.png');bpy.ops.render.render(write_still=True)
print('ACCESS_NATIVE_MODEL_COMPLETE: saved closed; rendered open inspection pose')
