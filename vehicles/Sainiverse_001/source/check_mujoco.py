...
for step in range(15000):
 ...
 for lift in lifts:
  ...
  for i,n in enumerate(names):
   ...
   gravity=0 if i==0 else -9.81*sum(lift['masses'][i:])*rot[2,2]
   d.ctrl[m.actuator(n).id]=np.clip(80000*(target[n]-q[i])-14000*dq[i]+gravity,-60000,60000)
  if step%200==0:rows.append(dict(time=t,name=lift['name'],out=float(q[0]),depth=float(q[1:].sum()),floor_z=float(d.xpos[m.body(names[-1]).id,2]+.125),underside_gap_m=float(d.xpos[m.body(names[-1]).id,2]-.125),ground_contacts=sum(1 for ct in d.contact if m.geom(names[-1]+"_deck").id in ct.geom)))
 e.substep(0.)
