...
- 交互驾驶使用 60 Hz。手动选择 MicroDuck 时，母车驻车，MD 在同一 Godot 物理世界内以 200 Hz 运行，接触实际雪地、甲板、升降梯和栏杆；Sai 使用 1000 Hz，机器人策略运行 50 Hz。独立进程中的移动母车 MD 仅供显式设置 `SAINIVERSE_REMOTE_MD_PREVIEW=1` 时预览，其平地替身不提供母车碰撞。完整路线与帧率数据见[验证记录](docs/VALIDATION.md#polar-game-integration-2026-09-20)。
